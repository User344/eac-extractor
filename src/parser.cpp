...
void c_parser::parse_driver( std::vector< parser_module_s >& modules ) const {
	...
	// This is basically std::vector in release
	// (I think we cant use std::vector here because it differs in debug)
	struct vector_s {
		void* begin;
		void* end;
		void* capacity;
	};
	using get_driver_t = bool( uint64_t, uint64_t, vector_s* );
	...
	module.name = "driver";
	module.ext = "sys";
	module.buffer.assign( reinterpret_cast< uint8_t* >( v.begin ),
						  reinterpret_cast< uint8_t* >( v.end ) );
...
void c_parser::parse_usermode( std::vector< parser_module_s >& modules ) const {
	...
	module.ext = "dll";
	module.buffer.resize( buffer_size );

	eac::decrypt( reinterpret_cast< void* >( buffer_addr ), buffer_size, module.buffer.data( ),
				  buffer_size );
